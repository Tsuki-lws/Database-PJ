...
该ER图展示了大语言模型评测平台数据库的实体关系结构，主要包含以下几个核心部分：

1. **用户管理**：users表存储用户信息，支持多种用户类型（管理员、普通用户、专家、审核员）
2. **问题管理**：包含原始问题(raw_questions)和标准问题(standard_questions)，以及相关的分类、标签系统
3. **答案管理**：包含原始答案(raw_answers)、标准答案(standard_answers)、众包答案(crowdsourced_answers)和专家答案(expert_answers)
4. **数据集管理**：通过dataset_versions和dataset_question_mapping管理不同版本的数据集
5. **模型评测**：包含模型(llm_models)、评测批次(evaluation_batches)、模型回答(llm_answers)和评测结果(evaluations)
6. **众包任务**：通过crowdsourcing_tasks管理众包任务，收集和评估答案

系统支持版本控制功能，对标准问题、标准答案、关键点和选项的修改都会记录历史版本。 